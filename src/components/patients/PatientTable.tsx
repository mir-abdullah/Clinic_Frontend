...
export const PatientTable = ({ patientsList }: { patientsList: Patient[] }) => {
    return (
                        ...
                        {patientsList.map((patient, index) => {
                            ...
                            const lastVisit = patient.visits && patient.visits.length > 0
                                ? patient.visits.reduce((latest, visit) => {
                                        ...
                                        return visitDate > new Date(latest.date) ? visit : latest;
                                    })
                                : null;
                            ...
                            return (
                                <TableRow key={patient.id} className="border-border/80 hover:bg-(--bg-secondary) transition-colors ">
                                    ...
                                    <TableCell className="text-(--text-primary) py-2">{patient?.phone || "N/A"}</TableCell>
