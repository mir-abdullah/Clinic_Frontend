...
export default async function PatientsPage({
  ...
  try {
    ...
    patients = { data: { patients: [], pagination: { totalPages: 1 } } };
  }

  const patientsList = patients?.data?.patients ?? [];
  ...
  return (
    <div>
      ...
      <div className="mt-10">
        <PatientTable patientsList={patientsList} />
        {totalPages > 1 && (
          <div className="mt-4">
            <Suspense fallback={null}>
              <VisitPagination currentPage={currentPage} totalPages={totalPages} />
            </Suspense>
          </div>
        )}
      </div>
